...
def get_vllm_pod_info(
    ...
    selectors = get_deployment_selectors(apps_v1, namespace, deployment_name)
    if len(selectors) == 0:
        raise RuntimeError(
...
def get_deployment_selectors(
    apps_v1: client.AppsV1Api, namespace: str, name: str
) -> list[str]:
...
def parse_gpu_logs(scenario: BenchmarkScenario, logs: list[LogLine]) -> None:
    """parse gpu logs"""
...
def benchmark_nop(
    ...
    namespace: str,
    endpoint_url: str,
    endpoint_launcher_url: str,
    endpoint_launcher_vllm_url: str,
    ...
    pod_info = None
    try:
        pod_info = get_vllm_pod_info(v1, apps_v1, namespace, arr[0])
