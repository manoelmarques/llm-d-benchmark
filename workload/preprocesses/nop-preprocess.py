...
"""Installs dependencies and moves cached files into or out of OS page cache"""
...
def scan_hf_cache(model: str) -> list[pathlib.Path]:
    ...
    for repo in hf_cache_info.repos:
        ...
        if "main" not in repo.refs:
            raise RuntimeError(
                f"HF Cache for model: {model} missing ref 'main'."
                f"It contains refs: {list(repo.refs.keys())}"
            )
...
def run_drop_page_cache(cached_files: list[pathlib.Path]):
    """runs vmtouch and drops page cache"""

    drop_page_cache_cmd = ["sudo -n sh -c 'sync; echo 1 > /proc/sys/vm/drop_caches'"]

    if len(cached_files) == 0:
        logger.info("not running vmtouch, no files found in hf cache")
    else:
        run_vmtouch(False, cached_files)

    cmd = drop_page_cache_cmd
    try:
        logger.info("start command: %s", " ".join(cmd))
        output, err, rc = minimal_ext_cmd(cmd, True)
        if rc > 0:
            raise RuntimeError(
                f"Command '{' '.join(cmd)}' exited with code '{rc}': '{err}'"
            )
...
def preprocess_run() -> str:
    ...
    model = envs[1].strip()
    in_page_cache = envs[2].strip().lower() in ["yes", "true"]
    ...
    if load_format == "tensorizer":
        ...
        # get files from HF Cache
        cached_files = scan_hf_cache(model)

    if in_page_cache:
        # run vmtouch to load model files into page cache
        if len(cached_files) == 0:
            logger.info(
                "not running vmtouch, no files found in hf cache model: %s", model
            )
        else:
            run_vmtouch(True, cached_files)
    else:
        # drop page cache
        run_drop_page_cache(cached_files)
