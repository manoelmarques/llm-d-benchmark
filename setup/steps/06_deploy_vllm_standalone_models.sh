...
EOF
  ...
  preprocessfile=$(find ${LLMDBENCH_MAIN_DIR}/workload/preprocesses -name ${LLMDBENCH_PREPROCESS_NAME}*)
  ...
  template:
    ...
    spec:
      ...
      - name: vllm-standalone-$(model_attribute $model label)
        ...
        - "-c"
        - |
          export LLMDBENCH_VLLM_STANDALONE_MODEL=$(model_attribute $model model)
          export LLMDBENCH_VLLM_TENSORIZER_URI=""
          extraargs="{}"
          if [[ \${LLMDBENCH_PREPROCESS_NAME} == "nop" ]]; then
            apt-get update -y
            apt-get install -y vmtouch
            # echo the in page cache status for nop harness
        ...
        - name: LLMDBENCH_PREPROCESS_NAME
          value: "${LLMDBENCH_PREPROCESS_NAME}"
        - name: HF_HOME
