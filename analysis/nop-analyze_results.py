...
def read_log_results_from_csv(file_path: str) -> list[LogResult]:
    ...
    if not os.path.isfile(file_path):
        logger.info("no csv file found path: %s", file_path)
        return log_results
...
def main():
    ...
    for root, _, files in os.walk(control_work_dir):
        for f in files:
            logger.info("File %s %s", root, f)
    ...
    cvs_filepath = os.path.join(control_work_dir, f"{analysis_filename}.csv")
    ...
    # write analysis file
    analysis_filepath = os.path.join(control_work_dir, f"{analysis_filename}.txt")
